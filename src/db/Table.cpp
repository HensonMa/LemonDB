...
void Table::deleteByIndex(const KeyType& key) {
  ...
  const size_t index = this->keyMap.find(key)->second;
...
void Table::duplicate(std::vector<Table::KeyType> &data_t, Table::SizeType & count) {
    for (auto & key : data_t) {
        const KeyType key_copy = key + "_copy";
        if (this->keyMap.find(key_copy) != this->keyMap.end()) {
            count --;
            continue;
        }
        auto data_copy = ((*this)[key])->it->datum;
        this->insertByIndex(key_copy, std::move(data_copy));
    }
}
