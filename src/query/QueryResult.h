...
class RecordAnsResult : public SucceededQueryResult {
    std::string msg;

public:
    bool display() override { return true; }

    explicit RecordAnsResult(const int number) {
        this->msg = R"(ANSWER = ?)"_f % number;
    }

    explicit RecordAnsResult(const std::vector<int>& results) {
        std::stringstream ss;
        ss << "ANSWER = ( ";
    ...
    explicit RecordAnsResult(const char *qname) {
        this->msg = R"("?")"_f % qname;
    }

    explicit RecordAnsResult(std::string qname) {
        this->msg = std::move(qname);
    }
