        ...
        if(result.second){
            if ((total_thread_num <= 1) || (table.size() < MAX_LINE)){
                ...
                for (int i = 0; i < total_thread_num-1; i++){
                    results[(unsigned)i] = thread_pool.worker(subWorker_add, i);
                }
                count += subWorker_add((int)total_thread_num-1);
                for (int i = 0; i < total_thread_num-1; i++)
