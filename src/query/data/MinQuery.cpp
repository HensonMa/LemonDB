...
QueryResult::Ptr MinQuery::execute(){
    ...
    try {
      ...
      rst_arr.clear();
      rst_arr.resize((this->operands).size(),INT32_MAX);
      copy_query  = this;
      ...
      if(result.second){
        if ((total_thread_num <= 1) || (table.size() < MAX_LINE)){
          ...
          for (int i = 0; i < total_thread_num-1; i++){
            results[(unsigned)i] = thread_pool.worker(subWorker_min, i);
          }
          subWorker_min((int)total_thread_num-1);
          for (int i = 0; i < total_thread_num-1; i++)
